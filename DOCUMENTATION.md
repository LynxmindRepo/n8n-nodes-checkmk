# n8n-nodes-checkmk Documentation
...
This node uses the Checkmk REST API for authentication. To use this node, you need to configure credentials in n8n with your Checkmk automation user details.

### Overview

The Checkmk node requires the following credential fields:

| Field | Description | Required | Example |
|-------|-------------|----------|---------|
| **Host URL** | The base URL of your Checkmk server (without trailing slash) | Yes | `https://monitoring.example.com` |
| **Site Name** | The name of your Checkmk site | Yes | `mysite` |
| **Username** | The automation user username | Yes | `automation-user` |
| **Password** | The automation user password/secret | Yes | `your-secret-key` |

### Creating an Automation User in Checkmk

Before configuring credentials in n8n, you need to create an automation user in Checkmk with REST API access:

1. **Log in to Checkmk** as an administrator
2. **Navigate to Users**:
   - Go to `Setup` → `Users` → `Users`
   - Or access directly via: `Setup` → `Users` → `Automation users`
3. **Create a new automation user**:
   - Click `Add automation user` or `Create automation user`
   - Enter a username (e.g., `n8n-automation`)
   - Set a secure password/secret
   - **Important**: Enable "REST API" permission/access
   - Configure appropriate permissions for the resources you need to access
4. **Save the user** and note the username and password/secret

### Finding Your Checkmk Site Name

The site name is typically visible in:
- The Checkmk URL: `https://your-checkmk-server/site-name/check_mk/`
- The Checkmk web interface header
- Checkmk configuration files
- Or ask your Checkmk administrator

Common site names include:
- `mysite` (default)
- `production`
- `monitoring`
- Or your organization's naming convention

### Configuring Credentials in n8n

1. **Open n8n** and navigate to `Credentials`
2. **Click** `Add Credential` or `New`
3. **Search for** `Checkmk API` or select it from the list
4. **Fill in the credential fields**:

   ```
   Host URL: https://monitoring.example.com
   Site Name: mysite
   Username: automation-user
   Password: your-secret-key
   ```

5. **Test the connection** (if available) to verify credentials work
6. **Save** the credentials with a descriptive name (e.g., `Checkmk Production`)

### Authentication Method

The node uses **Bearer token authentication** as required by the Checkmk REST API:

```
Authorization: Bearer {username} {password}
```

The authentication header is automatically generated by the node using the credentials you provide. The format combines the username and password as specified in the Checkmk REST API documentation.

### API Endpoint Construction

The node constructs API endpoints using your credentials:
...
**Resulting URL**: `https://monitoring.example.com/mysite/check_mk/api/1.0/domain-types/host_config/collections/all`

### Security Best Practices

1. **Use Dedicated Automation Users**:
   - Never use personal user accounts for automation
   - Create separate automation users for different purposes/environments
   - Use descriptive usernames (e.g., `n8n-production`, `n8n-staging`)

2. **Minimal Permissions**:
   - Grant only the minimum required permissions to automation users
   - Review and audit permissions regularly
   - Use different users for read-only vs. write operations if possible

3. **Secure Credential Storage**:
   - Store credentials securely in n8n's credential system
   - Never hardcode credentials in workflows
   - Use environment-specific credentials (production, staging, development)

4. **Regular Rotation**:
   - Rotate automation user passwords/secrets regularly
   - Update credentials in n8n when rotated
   - Document rotation schedule and process

5. **Network Security**:
   - Use HTTPS for Checkmk connections (never HTTP)
   - Restrict network access to Checkmk API if possible
   - Use VPN or private networks when available

### Credential Testing

The node includes automatic credential testing that:
- Verifies the connection to your Checkmk server
- Checks authentication with the provided credentials
- Validates the site name and API access

If credential testing fails, check:
- Host URL is correct and accessible
- Site name matches your Checkmk configuration
- Username and password are correct
- Automation user has REST API access enabled
- Network connectivity to Checkmk server

### Troubleshooting Credential Issues

#### Authentication Errors (401 Unauthorized)

**Symptoms**: `401 Unauthorized` errors when executing workflows

**Solutions**:
- Verify username and password are correct (no extra spaces or typos)
- Ensure the automation user has REST API access enabled in Checkmk
- Check that the site name is correct
- Verify the automation user hasn't been disabled or deleted
- Test credentials directly with Checkmk API using curl or Postman

**Test with curl**:
```bash
curl -X GET "https://monitoring.example.com/mysite/check_mk/api/1.0/version" \
  -H "Authorization: Bearer automation-user your-secret-key"
```

#### Connection Errors

**Symptoms**: Cannot connect to Checkmk server

**Solutions**:
- Verify the Host URL is correct (no trailing slash)
- Check network connectivity to the Checkmk server
...
**Symptoms**: `404 Not Found` or site-related errors

**Solutions**:
- Verify the site name is correct (case-sensitive)
- Check the site exists in your Checkmk installation
- Ensure the site name matches exactly (no extra spaces)
- Verify the automation user has access to the specified site

#### Permission Errors (403 Forbidden)

**Symptoms**: `403 Forbidden` errors on specific operations

**Solutions**:
- Verify the automation user has necessary permissions for the operation
- Check user permissions in Checkmk: `Setup` → `Users` → `Users` → [Your User] → `Permissions`
- Ensure the user has access to the required resources (hosts, services, etc.)
- Review Checkmk audit logs for detailed permission errors

### Multiple Environments

If you work with multiple Checkmk environments (production, staging, development), create separate credentials in n8n:

- `Checkmk Production` → Production server credentials
- `Checkmk Staging` → Staging server credentials
- `Checkmk Development` → Development server credentials

This allows you to:
- Use the same workflows across environments
- Switch environments easily
- Maintain separate security policies per environment

### Credential Examples

#### Production Environment
```
Host URL: https://monitoring.company.com
Site Name: production
Username: n8n-automation-prod
Password: [secure-secret-key]
```

#### Staging Environment
```
Host URL: https://monitoring-staging.company.com
Site Name: staging
Username: n8n-automation-staging
Password: [secure-secret-key]
```
...
- [Checkmk User Management](https://docs.checkmk.com/latest/en/user_management.html)
- [Checkmk Automation User Guide](https://docs.checkmk.com/latest/en/automation.html)
