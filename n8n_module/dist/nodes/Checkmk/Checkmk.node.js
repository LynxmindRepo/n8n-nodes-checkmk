        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (resource === 'rule') {
                    if (operation === 'get') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const ruleId = additionalFields.ruleId;
                ...
                if (resource === 'service') {
                    ...
                    if (operation === 'get') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const serviceDescription = additionalFields.serviceDescription;
                        const response = await GenericFunctions_1.checkmkApiRequest.call(this, 'GET', `/objects/service/${hostName}/${encodeURIComponent(serviceDescription)}`);
                        returnData.push(response);
                    }
                    if (operation === 'getMany') {
                        const returnAll = this.getNodeParameter('returnAll', i);
                ...
                if (resource === 'downtime') {
                    if (operation === 'create') {
                        ...
                        const body = {
                            ...
                            start_time: additionalFields.startTime
                                ? new Date(additionalFields.startTime).toISOString()
                                : new Date().toISOString(),
                            end_time: additionalFields.endTime
                                ? new Date(additionalFields.endTime).toISOString()
                    ...
                    if (operation === 'get') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const downtimeId = additionalFields.downtimeId;
                ...
                if (resource === 'biAggregation') {
                    if (operation === 'getState') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const filterNames = additionalFields.filterNames;
                        const filterGroups = additionalFields.filterGroups;
                        const qs = {};
                        if (filterNames)
                ...
                if (resource === 'comment') {
                    if (operation === 'create') {
                        ...
                        const comment = additionalFields.comment;
                        const persistent = additionalFields.persistent || false;
                        const body = {
                            comment,
                            persistent,
                        };
                    ...
                    if (operation === 'delete') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const commentId = additionalFields.commentId;
                ...
                if (resource === 'sla') {
                    if (operation === 'compute') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const slaConfiguration = additionalFields.slaConfiguration;
                        const timeRange = additionalFields.timeRange;
                        const body = {
                ...
                if (resource === 'auxTag') {
                    if (operation === 'create') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const tagId = additionalFields.tagId;
                ...
                if (resource === 'hostTagGroup') {
                    if (operation === 'create') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const tagGroupId = additionalFields.tagGroupId;
                        const title = additionalFields.title;
                        const topic = additionalFields.topic;
                        const help = additionalFields.help;
                        const body = {
                ...
                if (resource === 'ldapConnection') {
                    if (operation === 'create') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const connectionId = additionalFields.connectionId;
                        const serverUrl = additionalFields.serverUrl;
                        const bindDn = additionalFields.bindDn;
                        const bindPassword = additionalFields.bindPassword;
                        const body = {
                ...
                if (resource === 'openTelemetry') {
                    if (operation === 'create') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const collectorId = additionalFields.collectorId;
                        const name = additionalFields.name;
                        const port = additionalFields.port;
                        const body = {
                ...
                if (resource === 'samlConnection') {
                    if (operation === 'create') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const connectionId = additionalFields.connectionId;
                        const identityProviderMetadata = additionalFields.identityProviderMetadata;
                        const body = {
                            connection_id: connectionId,
                    ...
                    if (operation === 'update') {
                        ...
                        const response = await GenericFunctions_1.checkmkApiRequest.call(this, 'PUT', `/objects/saml_connection/${connectionId}`, body);
                        returnData.push(response);
                    }
                    if (operation === 'delete') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const connectionId = additionalFields.connectionId;
                        await GenericFunctions_1.checkmkApiRequest.call(this, 'DELETE', `/objects/saml_connection/${connectionId}`);
                ...
                if (resource === 'userRole') {
                    if (operation === 'create') {
                        const additionalFields = this.getNodeParameter('additionalFields', i, {});
                        const roleId = additionalFields.roleId;
                        const alias = additionalFields.alias;
                        const permissions = additionalFields.permissions;
                        const body = {
                            role_id: roleId,
                            alias,
                            permissions,
